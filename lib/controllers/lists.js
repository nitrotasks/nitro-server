...
lists.post('/', passport.authenticate('bearer', { session: false }), async function(req, res) {
  const query = {
  ...
  if (req.body.name.slice(0,9) === 'nitrosys-') {
    return res.status(403).send({message: 'Not allowed to create system lists.'})
  }
...
lists.get('/', passport.authenticate('bearer', { session: false }), async function(req, res) {
  const query = {
...
lists.delete('/', passport.authenticate('bearer', {session: false}), async function(req, res) {
  ...
  if (systemCheck) {
    return res.status(403).send({message: 'Not allowed to delete system lists.'})
  }
